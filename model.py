...
class FunASRNano(nn.Module):
    ...
    def inference(
        ...
        **kwargs,
        ...
        # 1. 检查是否开启自动切片 (默认开启，阈值设为 60秒)
        # 你可以在调用时传入 use_chunking=False 来关闭
        use_chunking = kwargs.get("use_chunking", True)
        chunk_duration_ms = kwargs.get("chunk_duration_ms", 60000) # 60秒一片
        
        # 简单判断：如果输入是文件路径且音频过长，则进入切片模式
        # 这里只演示处理 data_in 中只有一个文件的情况，这是最常见的用法
        is_long_audio = False
        if use_chunking and isinstance(data_in, list) and len(data_in) == 1 and isinstance(data_in[0], str):
             if os.path.exists(data_in[0]):
                # 预读一下时长，这里为了性能直接假设如果不指定就切片，或者你也可以用 torchaudio.info 判断
                is_long_audio = True
        if not is_long_audio:
            ...
            wav_path = data_in[0]
            logging.info(f"Detected long audio processing for: {wav_path}")
            ...
            if waveform.shape[0] > 1:
                waveform = torch.mean(waveform, dim=0) # 转单声道
            else:
            ...
            for start_idx in range(0, total_samples, chunk_samples):
                ...
                time_offset = start_idx / 16000.0 # 当前切片的起始时间(秒)
                
                # 构造 Prompt (每一段都当作一个新的语音输入)
                prompt = self.get_prompt(
                    kwargs.get("hotwords", []), kwargs.get("language", None), kwargs.get("itn", True)
                )
                
                # generate_chatml 支持直接传入 Tensor
                # 注意：这里传入的是单个 Tensor，inference_llm 期望的是 list
                chunk_data_in = [self.generate_chatml(prompt, audio_chunk)]
                
                # 生成随机 Key
                chunk_key = ["chunk_key_" + str(start_idx)]
                
                # 调用核心推理 (强制开启 fp16 节省显存，或者继承 kwargs)
                # 建议在切片时清理缓存
                torch.cuda.empty_cache()
                
                try:
                    res, _ = self.inference_llm(
                        chunk_data_in,
                        key=chunk_key,
                        tokenizer=tokenizer,
                        frontend=frontend,
                    ...
                    logging.error(f"Chunk processing failed at {time_offset}s: {e}")
                    continue
                
                if not res: continue
                
                segment = res[0] # 取第一个结果
                
                # 3. 合并结果
                # 文本直接拼接 (可能需要加空格，中文不需要)
                merged_result["text"] += segment.get("text", "")
                merged_result["text_tn"] += segment.get("text_tn", "")
                merged_result["ctc_text"] += segment.get("ctc_text", "")
                
                # 时间戳修正
                for ts_key in ["timestamps", "ctc_timestamps"]:
                    if ts_key in segment:
                        for item in segment[ts_key]:
                            # 复制一份避免引用问题
                            new_item = item.copy()
                            new_item["start_time"] += time_offset
                            new_item["end_time"] += time_offset
                            merged_result[ts_key].append(new_item)
